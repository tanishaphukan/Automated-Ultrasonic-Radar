/*
  ============================================================
  Automated Ultrasonic Radar - Processing IDE (Radar Display)
  Receives serial data from Arduino and draws a radar screen
  ============================================================

  HOW TO USE:
  1. Upload radar_arduino.ino to Arduino Uno first
  2. Check your COM port in Device Manager (Windows)
     or /dev/ttyUSB0 (Linux/Mac)
  3. Update the port name below to match yours
  4. Click Run in Processing IDE
  ============================================================
*/
...
// ---- UPDATE THIS TO YOUR COM PORT ----
String comPort = "COM3";   // e.g. "COM3" on Windows, "/dev/ttyUSB0" on Linux
...
// ============================================================
void draw() {
  // Fade effect for sweep trail
  fill(98, 245, 31);
  noStroke();
  fill(0, 4);
  rect(0, 0, width, height - height * 0.065);

  fill(98, 245, 31);   // radar green
  drawRadar();
  drawLine();
  drawObject();
  drawText();
...
void drawRadar() {
  ...
  // Draw range arcs
  arc(0, 0, (width - width * 0.0625) * 0.25, (width - width * 0.0625) * 0.25, PI, TWO_PI);
  arc(0, 0, (width - width * 0.0625) * 0.5,  (width - width * 0.0625) * 0.5,  PI, TWO_PI);
  arc(0, 0, (width - width * 0.0625) * 0.75, (width - width * 0.0625) * 0.75, PI, TWO_PI);
...
void drawText() {
  ...
  translate(width - width * 0.513 + width / 2 * cos(radians(120)),
            (height - height * 0.07) - width / 2 * sin(radians(120)));
  rotate(radians(-30));
  text("120°", 0, 0);
  popMatrix();
  ...
  translate((width - width * 0.5104) + width / 2 * cos(radians(150)),
            (height - height * 0.07) - width / 2 * sin(radians(150)));
  rotate(radians(-60));
  text("150°", 0, 0);
  popMatrix();
}
